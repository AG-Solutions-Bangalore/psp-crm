...
import TabbedTableGranualToYarn from "./TabbedTableYarnToFabric";
const fetchYarnToFabric = async (id, token) => {
...
const YarnToFabricProductionForm = () => {
  const { id } = useParams();
  ...
  const { data: YarnTFabric, isFetching } = useQuery({
    queryKey: ["yarntofabricId", decryptedId],
    queryFn: () => fetchYarnToFabric(decryptedId, token),
    enabled: !!decryptedId,
  });
  useEffect(() => {
    if (decryptedId && YarnTFabric?.data) {
      const raw = YarnTFabric.data;
      setFormData({
        yarn_to_fp_date: raw.yarn_to_fp_date || "",
      });

      const subItems = Array.isArray(YarnTFabric?.data?.subs)
        ? YarnTFabric?.data?.subs.map((sub) => ({
            id: sub.id || "",
            yarn_to_fp_sub_color_id: sub.yarn_to_fp_sub_color_id || "",
            yarn_to_fp_thickness: sub.yarn_to_fp_thickness || "",
            yarn_to_fp_weight: sub.yarn_to_fp_weight || "",
          }))
        : [
            {
              yarn_to_fp_thickness: "",
      ...
      const subItemsOne = Array.isArray(YarnTFabric?.data?.subs1)
        ? YarnTFabric?.data?.subs1.map((sub) => ({
            id: sub.id || "",
            fabric_from_p_color_id: sub.fabric_from_p_color_id || "",
            fabric_from_p_mtr: sub.fabric_from_p_mtr || "",
            fabric_from_p_weight: sub.fabric_from_p_weight || "",
            fabric_from_p_thickness: sub.fabric_from_p_thickness || "",
          }))
        : [
            {
              id: "",
  ...
  const handlePaymentChange = (selectedValue, rowIndex, fieldName) => {
    ...
    if (
      fieldName === "yarn_to_fp_thickness" ||
      fieldName === "yarn_to_fp_sub_color_id" ||
      fieldName === "yarn_to_fp_weight"
    ) {
      ...
      fieldName === "fabric_from_p_color_id" ||
  ...
  useEffect(() => {
    const calculateProgress = () => {
      ...
      const filledInvoiceFields = invoiceData.reduce((acc, item) => {
        return (
          acc +
          (item.yarn_to_fp_thickness?.toString().trim() !== "" ? 1 : 0) +
          (item.yarn_to_fp_sub_color_id?.toString().trim() !== "" ? 1 : 0) +
          (item.yarn_to_fp_weight?.toString().trim() !== "" ? 1 : 0)
        );
      }, 0);
  ...
  const handleSubmit = async (e) => {
    ...
    invoiceData.forEach((row, index) => {
      if (!row.yarn_to_fp_thickness)
        missingFields.push(`Row ${index + 1}: Bags`);
      if (!row.yarn_to_fp_sub_color_id)
        missingFields.push(`Row ${index + 1}: Color`);
      if (!row.yarn_to_fp_weight)
        missingFields.push(`Row ${index + 1}: Weight`);
    });

    invoiceDataOne.forEach((row, index) => {
      const {
        fabric_from_p_color_id: fabric_from_p_color_id,
        fabric_from_p_mtr: fabric_from_p_mtr,
        fabric_from_p_weight: fabric_from_p_weight,
        fabric_from_p_thickness: fabric_from_p_thickness,
      } = row;

      const isAnyFieldFilled =
        fabric_from_p_color_id?.toString().trim() !== "" ||
        fabric_from_p_mtr?.toString().trim() !== "" ||
        fabric_from_p_weight?.toString().trim() !== "";
      fabric_from_p_thickness?.toString().trim() !== "";

      if (isAnyFieldFilled) {
        if (!fabric_from_p_color_id)
          missingFields.push(`Fabric Row ${index + 1}: Color`);
        if (!fabric_from_p_mtr)
          missingFields.push(`Fabric Row ${index + 1}: Meter`);
        if (!fabric_from_p_weight)
          missingFields.push(`Fabric Row ${index + 1}: Weight`);
        if (!fabric_from_p_thickness)
    ...
    try {
      const hasValidGranualsData = invoiceDataOne.some((row) => {
        return (
          row.fabric_from_p_color_id?.toString().trim() !== "" ||
          row.fabric_from_p_mtr?.toString().trim() !== "" ||
          row.fabric_from_p_weight?.toString().trim() !== "" ||
          row.fabric_from_p_thickness?.toString().trim() !== ""
        );
      });
      ...
      const url = editId
        ? `${YARN_TO_FABRIC_PRODUCTION}/${decryptedId}`
  ...
  const handleDeleteYarn = async () => {
    try {
      const response = await apiClient.delete(
        `${YARN_TO_FABRIC_PRODUCTION}/sub/${deleteItemId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDeleteFabric = async () => {
    try {
      const response = await apiClient.delete(
        `${FABRIC_FROM_PRODUCTION}/${deleteItemId1}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <Page>
      <div className="p-0">
        <div className="">
          <form onSubmit={handleSubmit} className="w-full ">
            <PageHeaders
              title={
                editId ? "Update Yarn Production" : "Create Yarn Production"
              }
                  ...
                  <div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleInputChange(e, "yarn_to_fp_date")
                        }
                        type="date"
                      />
                    </div>
      ...
      <DeleteAlertDialog
        ...
        description="Fabric Production Sub"
...
export default YarnToFabricProductionForm;
