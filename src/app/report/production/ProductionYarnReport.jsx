...
import { YARN_PRODUCTION_REPORT } from "@/api";
...
const ProductionYarnReport = () => {
  ...
  const { data: yarnData, isLoading } = useQuery({
    queryKey: ["yarnData", searchParams],
    queryFn: async () => {
      ...
      try {
        ...
        console.error("Error fetching yarn production report:", error);
        throw error;
      }
    },
  ...
  const yarnProductionData = yarnData?.data || [];

  const groupByMonth = () => {
    const grouped = {};
    yarnProductionData.forEach(item => {
      const date = new Date(item.granuals_to_yp_date);
  ...
  const calculateTotals = (data) => {
    return data.reduce((acc, item) => {
      acc.totalBags += Number(item.yarn_from_p_bags || 0);
      acc.totalWeight += Number(item.yarn_from_p_weight || 0);
      acc.totalThickness += Number(item.yarn_from_p_thickness || 0);
      return acc;
    }, { totalBags: 0, totalWeight: 0, totalThickness: 0 });
  };

  const handlePrintPdf = useReactToPrint({
    content: () => containerRef.current,
    documentTitle: "Yarn Production Report",
    pageStyle: `
      @page {
        size: A4 portrait;
        margin: 5mm;
  ...
  const downloadAllCSV = async (data, toast, setExcelLoading) => {
    ...
    try {
      ...
      const getRowData = (item) => [
        moment(item.granuals_to_yp_date).format("DD-MM-YYYY"),
        item.color || "",
        Number(item.yarn_from_p_bags || 0),
        Number(item.yarn_from_p_weight || 0).toFixed(2),
        Number(item.yarn_from_p_thickness || 0).toFixed(2)
      ];
      ...
      await downloadExcelMultiRow({
        data: excelData,
        sheetName: "Yarn Production Report",
        fileNamePrefix: "yarn_production_report",
        toast,
        customFormat: true,
        emptyDataCallback: () => ({
          title: "No Data",
  ...
  const downloadMonthwiseCSV = async (monthlyData, toast, setExcelLoading) => {
    ...
    try {
      await downloadExcelMultiRow({
        data: allData,
        sheetName: "Monthwise Yarn Production",
        fileNamePrefix: "monthwise_yarn_production",
        toast,
        customFormat: true,
        emptyDataCallback: () => ({
          title: "No Data",
  ...
  const downloadColorwiseCSV = async (colorData, toast, setExcelLoading) => {
    ...
    try {
      await downloadExcelMultiRow({
        data: allData,
        sheetName: "Colorwise Yarn Production",
        fileNamePrefix: "colorwise_yarn_production",
        toast,
        customFormat: true,
        emptyDataCallback: () => ({
          title: "No Data",
  ...
  const allTotals = calculateTotals(yarnProductionData);
        ...
        <div className={`sticky top-0 z-10 border border-gray-200 rounded-lg ${ButtonConfig.cardheaderColor} shadow-sm p-3 mb-2`}>
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            ...
            <div className="w-[30%] shrink-0">
              <h1 className="text-xl font-bold text-gray-800 truncate">Yarn Production Report</h1>
              <p className="text-xs text-gray-600">View yarn production data</p>
            </div>
        ...
        {searchParams && (
          <Tabs defaultValue="all" className="w-full mt-4" onValueChange={setActiveTab}>
            ...
            <TabsContent value="all">
              <div className="flex justify-end gap-2 mb-2">
                ...
                <Button
                  ...
                  className={`${ButtonConfig.backgroundColor} ${ButtonConfig.hoverBackgroundColor} ${ButtonConfig.textColor} flex items-center`}
                  onClick={() => downloadAllCSV(yarnProductionData, toast, setExcelLoading)}
                >
                  {excelloading ? (
                    <Loader className="animate-spin h-3 w-3" />
                  ) : (
              ...
              <div className="overflow-x-auto text-[11px] grid grid-cols-1" ref={containerRef}>
                ...
                <table className="w-full border-collapse border border-black">
                  ...
                  <tbody>
                    {yarnProductionData.length > 0 ? (
                      yarnProductionData.map((item, index) => (
                        <tr key={index} className="hover:bg-gray-50">
                          <td className="border border-black px-2 py-2">{moment(item.granuals_to_yp_date).format("DD-MM-YYYY")}</td>
                          <td className="border border-black px-2 py-2">{item.color}</td>
                          <td className="border border-black px-2 py-2 text-right">{item.yarn_from_p_bags}</td>
                          <td className="border border-black px-2 py-2 text-right">{Number(item.yarn_from_p_weight || 0).toFixed(2)}</td>
                          <td className="border border-black px-2 py-2 text-right">{Number(item.yarn_from_p_thickness || 0).toFixed(2)}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
            ...
            <TabsContent value="monthwise">
              ...
              <div className="overflow-x-auto text-[11px] grid grid-cols-1" ref={containerRef}>
                <h1 className={`text-center text-2xl font-semibold mb-3 print:block`}>
                  Yarn Production Report (Monthwise)
                </h1>
            ...
            <TabsContent value="colorwise">
              ...
              <div className="overflow-x-auto text-[11px] grid grid-cols-1" ref={containerRef}>
                <h1 className={`text-center text-2xl font-semibold mb-3 print:block`}>
                  Yarn Production Report (Colorwise)
                </h1>
...
export default ProductionYarnReport;