...
const SalesYarnReport = () => {
  ...
  const fabricData = salesData?.data?.filter(item => item.sales_type === "Yarn") || [];
  ...
  const handlePrintPdf = useReactToPrint({
    ...
    documentTitle: "Yarn Sales Report",
    pageStyle: `
      @page {
        size: A4 portrait;
        margin: 5mm;
...
const downloadAllCSV = async (data, toast, setExcelLoading, companyStateName) => {
  ...
  const downloadMonthwiseCSV = async (monthlyData, toast, setExcelLoading, companyStateName, allTotals) => {
    ...
    try {
      await downloadExcelMultiRow({
        data: allData,
        sheetName: "Monthwise Yarn Sales",
        fileNamePrefix: "monthwise_yarn_sales",
        toast,
        customFormat: true,
        emptyDataCallback: () => ({
          title: "No Data",
  ...
  const downloadVendorwiseCSV = async (vendorData, toast, setExcelLoading, companyStateName, allTotals) => {
    ...
    try {
      await downloadExcelMultiRow({
        data: allData,
        sheetName: "Vendorwise Yarn Sales",
        fileNamePrefix: "vendorwise_yarn_sales",
        toast,
        customFormat: true,
        emptyDataCallback: () => ({
          title: "No Data",
          ...
          <div className={`sticky top-0 z-10 border border-gray-200 rounded-lg ${ButtonConfig.cardheaderColor} shadow-sm p-3 mb-2`}>
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
              ...
              <div className="w-[30%] shrink-0">
                <h1 className="text-xl font-bold text-gray-800 truncate">Yarn Sales Report</h1>
                <p className="text-xs text-gray-600">View yarn sales data</p>
              </div>
...
export default SalesYarnReport;