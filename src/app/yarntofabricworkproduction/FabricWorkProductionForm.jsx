...
const fetchYarnToFabricworkById = async (id, token) => {
...
const FabricWorkProductionForm = () => {
  const { id } = useParams();
  ...
  const { data: YarnToFabricWorkById, isFetching } = useQuery({
    queryKey: ["yarntofabricworkById", decryptedId],
    queryFn: () => fetchYarnToFabricworkById(decryptedId, token),
    enabled: !!decryptedId,
  });
  useEffect(() => {
    if (decryptedId && YarnToFabricWorkById?.data) {
      const raw = YarnToFabricWorkById.data;
      setFormData({
        year: financialYear || "",
        yarn_to_fwp_date: raw.yarn_to_fwp_date || "",
        yarn_to_fwp_ref: raw.yarn_to_fwp_ref || "",
      });
    }
  }, [decryptedId, YarnToFabricWorkById]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.yarn_to_fwp_ref) missingFields.push("Ref");

    invoiceData.forEach((row, index) => {
      if (!row.fabric_from_wp_color_id)
        missingFields.push(`Row ${index + 1}: Color`);
      if (!row.fabric_from_wp_mtr)
        missingFields.push(`Row ${index + 1}: Meter`);
      if (!row.fabric_from_wp_weight)
        missingFields.push(`Row ${index + 1}: Weight`);
      if (!row.fabric_from_wp_thickness)
    ...
    try {
      ...
      const response = await apiClient.post(FABRIC_WORK_PRODUCTION, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  if (isFetching || loadingitem) {
    return <LoaderComponent name="Yarn Work" />;
  }
  return (
    <Page>
      <div className="p-0">
        <div className="">
          <form onSubmit={handleSubmit} className="w-full ">
            <PageHeaders
              title={"Create Yarn Work Production"}
              subtitle="yarn"
              progress={progress}
            />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2  gap-2">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      value={formData.yarn_to_fwp_ref}
                      onChange={(e) =>
                          ...
                          <TableCell className="px-4 py-3 align-top">
                            <div className="flex flex-col gap-1">
                              <MemoizedProductSelect
                                value={row.fabric_from_wp_color_id}
                                onChange={(e) =>
                                  handlePaymentChange(
                                    e,
                                    rowIndex,
                          ...
                          <TableCell className="px-4 py-3 align-top">
                            <div className="flex flex-col gap-1">
                              <Input
                                ...
                                value={
                                  invoiceData[rowIndex]?.fabric_from_wp_mtr ||
                                  ""
                                }
...
export default FabricWorkProductionForm;
