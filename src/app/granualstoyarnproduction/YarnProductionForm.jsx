...
const YarnProductionForm = () => {
  ...
  useEffect(() => {
    if (decryptedId && GranualProductionById?.data) {
      ...
      setFormData({
        ...
        granuals_to_yp_ref: raw.granuals_to_yp_ref || "",
      });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.granuals_to_yp_ref) missingFields.push("Ref");

    invoiceData.forEach((row, index) => {
      if (!row.yarn_from_p_color_id)
        missingFields.push(`Row ${index + 1}: Color`);
      if (!row.yarn_from_p_bags) missingFields.push(`Row ${index + 1}: Bags`);
      if (!row.yarn_from_p_weight)
        missingFields.push(`Row ${index + 1}: Weight`);
      if (!row.yarn_from_p_thickness)
    ...
    try {
      ...
      const response = await apiClient.post(YARN_FROM_PRODUCTION, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
                ...
                <div className="grid grid-cols-1 md:grid-cols-2  gap-2">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      value={formData.granuals_to_yp_ref}
                      onChange={(e) =>
                          ...
                          <TableCell className="px-4 py-3 align-top">
                            <div className="flex flex-col gap-1">
                              <MemoizedProductSelect
                                value={row.yarn_from_p_color_id}
                                onChange={(e) =>
                                  handlePaymentChange(
                                    e,
                                    rowIndex,
                          ...
                          <TableCell className="px-4 py-3 align-top">
                            <div className="flex flex-col gap-1">
                              <Input
                                ...
                                value={
                                  invoiceData[rowIndex]?.yarn_from_p_bags || ""
                                }
                          ...
                          <TableCell className="px-4 py-3 align-top">
                            <div className="flex flex-col gap-1">
                              <Input
                                ...
                                value={
                                  invoiceData[rowIndex]?.yarn_from_p_weight ||
                                  ""
                                }
...
export default YarnProductionForm;
